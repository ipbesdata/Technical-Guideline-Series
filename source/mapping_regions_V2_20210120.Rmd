...
```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri("C:/Users/jkumagai/Documents/IPBES/R/Geoinformatics/Technical Guidelines Series/Mapping Guidelines v2/logo2.jpg"), 
               alt = 'logo', 
...
ocean <- st_transform(ocean, crs = crs_robin) # changes the projection
ocean <- ocean[,1]
```
...
Secondly, we aggregate the data per IPBES regions and sub-regions. In our example, I calculate the toal population per region and per sub-region using the group_by() function within the tidyverse package. 
```{r}
pop_2018 <- pop_2018 %>% 
...
data_region <- data %>% # this dissolves the data by region
  group_by(Region.x) %>% 
  summarise(region_pop2 = sum(Value, na.rm = T)/1000) %>% 
  st_cast() 

data_subregion <- data %>% # this dissolves the data by subregion
  group_by(Sub_Region.x) %>% 
  summarise(sub_region_pop2 = sum(Value, na.rm=T)/1000) %>% 
  st_cast()
```
...
labs <- graticule_labels(lons = long, lats = lat, xline = -180, yline = 90, proj = crs_robin) # labels for the graticules 
lines <- graticule(lons = long, lats = lat, proj = crs_robin) # graticules 
```
...
plot(ocean, col = alpha("slategray1", 0.3), add = TRUE) 
